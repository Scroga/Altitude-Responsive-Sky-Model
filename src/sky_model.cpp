...
double dot(const SkyModel::Vector3 &v1, const SkyModel::Vector3 &v2) {
	return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
}

double magnitude(const SkyModel::Vector3 &vector) {
	return std::sqrt(dot(vector, vector));
}

SkyModel::Vector3 normalize(const SkyModel::Vector3 &vector) {
...
void SkyModel::readRadiance(FILE *handle, const double singleVisibility) {
	// Read metadata.
...
void SkyModel::readTransmittance(FILE *handle) {
	// Read metadata
...
void SkyModel::readPolarisation(FILE *handle) {
	// Read metadata.
...
// Initialization
/////////////////////////////////////////////////////////////////////////////////////
void SkyModel::initialize(const std::string &filename, const double singleVisibility) {
...
SkyModel::AvailableData SkyModel::getAvailableData() const {
	if (!initialized) {
...
SkyModel::FrameInterpolationParameters SkyModel::computeFrameInterpolationParameters(
...
SkyModel::PixelInterpolationParameters SkyModel::computePixelInterpolationParameters(
		const Vector3 &viewDirection) const {
	const Metadata &metadata = metadataRad;
	...
	if (frameParams.distanceToView > PLANET_RADIUS) {
		...
		const double correction =
				sqrt(frameParams.distanceToView * frameParams.distanceToView - PLANET_RADIUS * PLANET_RADIUS) / frameParams.distanceToView;
...
std::vector<float>::const_iterator SkyModel::getCoefficients(const std::vector<float> &dataset,
...
SkyModel::InterpolationParameter SkyModel::getInterpolationParameter(
