...
var sky_shader_path: String
var sky_parameters_script: Script
var sky_dome_settings_script: Script
...
func _ensure_parameters() -> bool:
	if parameters != null:
		return true

	if sky_parameters_script == null:
		_initialize_resources()

	if sky_parameters_script == null:
		push_error("Failed to load parameters.gd")
		return false

	parameters = sky_parameters_script.new() as SkyParameters

	if parameters == null:
		push_error("Failed to instantiate SkyParameters")
		return false
...
func _initialize() -> void:
	...
	if parameters == null:
		parameters = sky_parameters_script.new()
...
func _initialize_resources() -> void:
	...
	sky_shader_path = addon_root.path_join(
		"shaders/sky_material.gdshader"
	)

	sky_parameters_script = load(
		addon_root.path_join("src/parameters.gd")
	) as Script

	sky_dome_settings_script = load(
		addon_root.path_join("src/sky_dome_settings.gd")
	) as Script
